...
REPORT_PATH = "/home/tparng/kev-gpt/model/tinystories_hf_repro/repetition_audit_report.html"

data = json.load(open(RESULTS_PATH))

payload = {
    "sweep1": {"C": data["sweep1"]["C"], "reference": data["sweep1"]["reference"]},
    "sweep2": {"C": data["sweep2"]["C"], "reference": data["sweep2"]["reference"]},
}
json_blob = json.dumps(payload).replace("</script", "<\\/script")

HTML = r"""<title>Repetition Audit</title>
<style>
:root{
...
<div class="wrap">
  ...
  <header class="top">
    ...
    <h1>Repetition Audit</h1>
    <p class="dek">Every generated story from a two-sweep, multi-seed comparison of
      <span class="modeltag c"><span class="dot c"></span>checkpoint C</span> (kev-gpt, the
      ten-step ablation walk's payoff checkpoint) against the published
    ...
    <div class="provenance">
      ...
      <span><b>C sampling</b> temp=0.8, top_k=40</span>
  ...
  <footer class="endnote">
    Generated fresh (not the log's own recorded numbers) via
    <code>data/ckpt_stepC_d384_v16384_fp.pt</code> (iter 15000, D=384/n_layer=12/n_head=6)
    vs <code>SauravP97/tiny-stories-19M</code>, same objective detector, each model sampled
    at its own established convention. Rows pair identical (seed, prompt-theme) conditions
    across models for direct comparison.
  </footer>
...
function renderStats(){
  const groups = [
    {key:'sweep1', model:'C', label:'Sweep 1 &middot; C', cls:'c'},
    {key:'sweep1', model:'reference', label:'Sweep 1 &middot; reference', cls:'r'},
    {key:'sweep2', model:'C', label:'Sweep 2 &middot; C', cls:'c'},
  ...
  const s1c = computeStats(DATA.sweep1.C), s1r = computeStats(DATA.sweep1.reference);
  const s2c = computeStats(DATA.sweep2.C), s2r = computeStats(DATA.sweep2.reference);
  ...
  document.getElementById('metrics-row').innerHTML = `
    <div class="metric-cell">
      <div class="mlabel">Sweep 1 (60 tok) &middot; per-sample rate, C vs reference</div>
      <table>
        <tr><td class="k"></td><td class="v" style="color:var(--accent-c)">C</td><td class="v" style="color:var(--accent-r)">ref</td></tr>
    ...
    <div class="metric-cell">
      <div class="mlabel">Sweep 2 (250 tok) &middot; per-sample rate, C vs reference</div>
      <table>
        <tr><td class="k"></td><td class="v" style="color:var(--accent-c)">C</td><td class="v" style="color:var(--accent-r)">ref</td></tr>
...
function pairRows(sweepKey){
  const cList = DATA[sweepKey].C, rList = DATA[sweepKey].reference;
  const rows = [];
...
function render(){
  ...
  rows.forEach((pair, idx) => {
    ...
    html += `<div class="rowgroup">
      ...
      <div class="pair">
        ${panelHtml(pair.c, 'c', 'checkpoint C')}
